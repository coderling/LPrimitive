...
{
template <>
struct hash<Toy::IUUID>
{
   public:
    size_t operator()(const Toy::IUUID& uuid) const
    {
        size_t hash_va = 0;
        Toy::Engine::ComputeHash(hash_va, uuid.data.d1, uuid.data.d3, uuid.data.d3);
        for (int i = 0; i < 8; ++i)
            {
                Toy::Engine::HashCombine(hash_va, uuid.data.array[i]);
            }
