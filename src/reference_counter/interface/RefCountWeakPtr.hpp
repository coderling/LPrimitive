...
class RefCountWeakPtr final
{
   ...
   public:
          ...
          p_refcounter(nullptr)
    {
        if (p_object != nullptr)
            {
                p_refcounter = Debug::StaticPointerCast<ReferenceCounter, IReferenceCounter>((p_object)->GetReferenceCounter());
            }
    }

    ~RefCountWeakPtr() noexcept { Release(); }
        ...
        if (p_refcounter != nullptr)
            {
                ...
                p_refcounter->GetObject(static_cast<IObject**>(&p_owner));
                if (p_owner != nullptr)
                    {
                        strong_ptr = p_object;
                    }
