...
{
    return DT::Primitive::GetGlobalAllocator().Allocate(n, alignof(std::max_align_t), 0, flags);
}

inline void* allocator::allocate(size_t n, size_t alignment, size_t offset, int flags)
{
    return DT::Primitive::GetGlobalAllocator().Allocate(n, alignment, offset, flags);
}

inline void allocator::deallocate(void* p, size_t n) { return DT::Primitive::GetGlobalAllocator().Free(p, n); }
