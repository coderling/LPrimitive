...
VariableSizeAllocationsManager::~VariableSizeAllocationsManager()
{
    ...
    L_ASSERT(free_block_by_offset.size() == 1, "must free all allocation");
    L_ASSERT(free_block_by_offset.size() == free_block_by_size.size(), "size of two map must equal!");
    L_ASSERT(free_block_by_offset.begin()->first == 0, "single block offset must to be 0");
    L_ASSERT(free_block_by_offset.begin()->second.size == max_size, "single block size must to be ", max_size);
    L_ASSERT_EXPR(free_block_by_offset.begin()->second.order_size_it == free_block_by_size.begin());

    L_ASSERT(free_block_by_size.begin()->first == max_size, "single block size must to be ", max_size);
    L_ASSERT(free_block_by_size.begin()->second == free_block_by_offset.begin(), "error iterator");
#endif
}
                                                                                    ...
                                                                                    const OffsetSizeType& alignment)
{
    ...
    L_ASSERT_EXPR(IsPowerOfTow(alignment), "alignment (", alignment, ") must be power of 2");
    ...
    L_ASSERT_EXPR(lower_bound_iter->second.size >= rsize + alignment_diff);
    L_ASSERT_EXPR(lower_bound_iter->second.size == lower_bound_iter_iter->first);
    L_ASSERT_EXPR(lower_bound_iter->second.order_size_it == lower_bound_iter_iter);
    // block:
    //    |---------------------block.size----------------------|
    ...
    if ((rsize & (current_alignment - 1)) != 0)
        {
            if (IsPowerOfTow(rsize))
                {
                    L_ASSERT_EXPR(rsize >= alignment && size < current_alignment);
...
{
    L_ASSERT_EXPR(allocation.IsValid());
...
{
    L_ASSERT_EXPR(offset + size <= max_size && offset != Allocation::InvalidAddressOffset);
    ...
    L_ASSERT_EXPR(next_block_iter == l_next_block_iter);
#endif
    // check overlap with nextblock
    L_ASSERT_EXPR(next_block_iter == free_block_by_offset.end() || offset + size <= next_block_iter->first);
    auto pre_block_iter = next_block_iter;
    if (pre_block_iter != free_block_by_offset.begin())
        {
            --pre_block_iter;
            // check overlap with preblock
            L_ASSERT_EXPR(pre_block_iter->first + pre_block_iter->second.size <= offset);
        }
    else
        {
    ...
    if (IsEmpty())
        {
            L_ASSERT_EXPR(NumOfFreeBlocks() == 1);
            ResetCurrentAlignment();
        }
            ...
            if (last_offset + last_size == max_size)
                {
                    ...
                    L_ASSERT_EXPR(last_iter->second.order_size_it->first == last_size &&
...
void VariableSizeAllocationsManager::DebugAllocation()
{
    ...
    L_ASSERT_EXPR(IsPowerOfTow(current_alignment));
    // size of two map must equal
    L_ASSERT_EXPR(free_block_by_size.size() == free_block_by_offset.size());
    ...
    while (block_iter != free_block_by_offset.end())
        {
            ...
            L_ASSERT_EXPR(block_iter->first + block_iter->second.size <= max_size);
            // offset must be aligned by current_alignment
            L_ASSERT_EXPR((block_iter->first & (current_alignment - 1)) == 0,
            ...
            if (block_iter->second.size + block_iter->first < max_size)
                {
                    ...
                    L_ASSERT_EXPR((block_iter->second.size & (current_alignment - 1)) == 0, "all block size(", block_iter->first, ",",
            ...
            L_ASSERT_EXPR(block_iter == block_iter->second.order_size_it->second);
            // block size equal size map iterator->first
            L_ASSERT_EXPR(block_iter->second.size == block_iter->second.order_size_it->first);
            // check block overlap
            L_ASSERT_EXPR(
    ...
    while (order_size_iter != free_block_by_size.end())
        {
            ...
            L_ASSERT_EXPR(order_size_iter->first == order_size_iter->second->second.size);
            total_free_size += order_size_iter->first;
            ++order_size_iter;
        }

    L_ASSERT_EXPR(total_free_size == free_size);
}
#endif
...
void VariableSizeAllocationsManager::AddNewBlock(const OffsetSizeType& offset, const OffsetSizeType& size)
{
    ...
    L_ASSERT_EXPR(block_iter.second);
