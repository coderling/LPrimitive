   ...
   public:
    ...
    TOY_RESULT ENGINE_FUNCTION_CALL_CONVENTION QueryInterface(const IUUID& iid, IObject** pp_interface) override
    {
        if (pp_interface == nullptr)
            {
                ENGINE_DEV_CHECK_EXPR(false, "Fail to QueryInterface (", iid.name, ")pp_interface = nullptr!");
        ...
        ENGINE_DEV_CHECK_EXPR(false, "Fail to QueryInterface ", iid.name, "!");
        return TOY_RESULT::TR_ERROR;
    }
};
...
#define IMPLEMENT_CONSTRUCT_DEFINE_HEAD(CLASS_TYPE, ...) CLASS_TYPE::CLASS_TYPE(IReferenceCounter* p_refcounter __VA_OPT__(,)  ##__VA_ARGS__) noexcept
