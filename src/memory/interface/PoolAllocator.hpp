...
class FreeList
{
   ...
   public:
    FreeList(void* _begin, void* _end, size_t element_size, size_t alignment) noexcept
    {
        void* const b_ptr = AlignUp(_begin, alignment);
...
class ThreadSafeFreeList
{
   ...
   public:
    ThreadSafeFreeList(void* begin, void* end, size_t element_size, size_t alignment) noexcept
    {
        void* const b_ptr = AlignUp(begin, alignment);
        void* const next = AlignUp(PtrAdd(b_ptr, element_size), alignment);
...
template <size_t ELEMENT_SIZE, size_t ALIGNMENT = alignof(std::max_align_t), typename FreeListType = FreeList>
class PoolAllocator
{
   ...
   public:
    PoolAllocator(void* begin, void* end) noexcept
        : list{begin, end, ELEMENT_SIZE, ALIGNMENT}
    {
    }

    template <typename Resource>
    explicit PoolAllocator(Resource& resource) noexcept
        : list{resource.GetBasePtr(), resource.GetEndPtr(), ELEMENT_SIZE, ALIGNMENT}
    {
    }
    ...
    void* const GetBasePtr() const noexcept { return list.First(); }
};

template <typename T, size_t ELEMENT_SIZE>
using ObjectPool = PoolAllocator<ELEMENT_SIZE, alignof(T), FreeList>;

template <typename T, size_t ELEMENT_SIZE>
using ThreadSafeObjectPool = PoolAllocator<ELEMENT_SIZE, alignof(T), ThreadSafeFreeList>;
