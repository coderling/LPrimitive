...
#define ARRAY_PUSH(arr, va) (DT::Primitive::ArrayHandler::ArrayCheckPush(&(arr), (va)))

#define ARRAY_EMPLACE_BACK(arr, ...) (DT::Primitive::ArrayHandler::ArrayCheckEmplaceBack(&(arr)__VA_OPT__(, )##__VA_ARGS__))

#define ARRAY_DELN(arr, index, count) (DT::Primitive::ArrayHandler::ArrayDel(&(arr), (index), (count)))

#define ARRAY_DEL(arr, index) (DT::Primitive::ArrayHandler::ArrayDel(&(arr), (index)))

#define ARRAY_FREE(arr) (DT::Primitive::ArrayHandler::ArrayFree(&(arr)))

#define ARRAY_LENGTH(arr) ((arr) ? DT::Primitive::ArrayHandler::ArrayHead((arr))->length : 0)

#define ARRAY_CAPACITY(arr) ((arr) ?: DT::Primitive::ArrayHandler::ArrayHead((arr))->capacity : 0)

#define ARRAY_SETLENGTH(arr, length) (DT::Primitive::ArrayHandler::ArraySetLength(&(arr), length))
};
